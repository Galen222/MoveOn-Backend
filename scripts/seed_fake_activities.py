...
def construir_actividad(
    indice_usuario: int, indice_actividad: int
) -> schemas.GuardarActividad:
    """
    Construye una actividad válida para el esquema actual.
    """
    tipo = (
        TipoActividad.CAMINAR
        if (indice_usuario + indice_actividad) % 2 == 0
        else TipoActividad.CORRER
    )
    ...
    fecha_ruta = ahora_utc() - timedelta(
        days=dias_atras, hours=horas_atras, minutes=random.randint(0, 59)
    )
...
async def crear_actividades() -> None:
    ...
    async with database.AsyncSessionLocal() as db:
        ...
        if not usuarios:
            print(
                "No se han encontrado usuarios usuario1@prueba.com ... usuario20@prueba.com"
            )
        ...
        for indice_usuario, usuario in enumerate(usuarios, start=1):
            ...
            print(
                f"[USER] {usuario.nombre_usuario}: {creadas_usuario} actividades creadas"
            )
